...
/// «Приложение» — про сам продукт, «Служба» — работает ли всё это сейчас.
struct SettingsTab {
    let id: String
    let ru: String
    let en: String
    /// Вкладка без единой настройки. Пустая вкладка — обычно дефект (так
    /// жила «Модель»), поэтому исключение объявляется явно и с причиной:
    /// самотест иначе требует от каждой вкладки хотя бы одну строку реестра.
    let holdsNoSettings: String?

    init(id: String, ru: String, en: String, holdsNoSettings: String? = nil) {
        self.id = id
        self.ru = ru
        self.en = en
        self.holdsNoSettings = holdsNoSettings
    }
...
let SETTINGS_TABS: [SettingsTab] = [
    ...
    .init(id: "service", ru: "Служба", en: "Service",
          holdsNoSettings: "Состояние, а не предпочтения: разрешения, модель, "
                         + "измерения и обслуживание. Переключать нечего, и "
                         + "подпись сверху снимает обещание строки вкладок. "
                         + "Отозванное разрешение зовёт сюда подвалом сайдбара "
                         + "и баннером «текст не вставился», а не точкой на "
                         + "ярлыке: точка приберегается для новости — "
                         + "доступного обновления во вкладке «Приложение»."),
]
